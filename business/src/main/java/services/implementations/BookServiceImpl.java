...
public class BookServiceImpl extends CommonServiceImpl<Book, Long> implements IBookService {
    ...
    public Set<Book> getAuthorBooks(String authorName) {
        Set<Book> books = bookRepository.findAllByAuthorName(authorName);
        books.forEach(book -> book.setDescription(descriptionService.getByBookId(book.getId())));
        return books;
    ...
    public Set<Book> findBooksByTitleIn(List<String> titles) {
        Set<Book> foundBooksByTitle = bookRepository.findBooksByTitleIn(titles);
        if (foundBooksByTitle.size() < titles.size()) {
            titles.removeAll(foundBooksByTitle.stream().map(book -> book.getTitle()).collect(Collectors.toList()));
            throw new NotFoundEntityException("These books not found: " + titles);
        }
    ...
    public Book updateFromJson(Long bookId, String jsonBook) throws IOException {
        ...
        if (map.size() != 0) {
            ...
            if (map.containsKey("bookOnHands")) {
                currentBook.setBookOnHands(bookChanges.getBookOnHands());
            }
