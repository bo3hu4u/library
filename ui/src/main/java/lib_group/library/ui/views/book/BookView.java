...
public class BookView extends VerticalLayout implements KeyNotifier {
    ...
    public BookView(IBookService bookService) {
        ...
        grid.addComponentColumn(book -> {
            ...
            onHands.setValue(book.getBookOnHands());
            ...
            return hlOnHandsBox;
        }).setHeader("On hands").setKey("bookOnHands").setComparator(comparator -> comparator.getBookOnHands()).setSortable(true);
    ...
    private void setFiltersToColumns() {
        ...
        filterOnHands.addValueChangeListener(valueChangeEvent -> {
            ((ListDataProvider<Book>) grid.getDataProvider()).addFilter(book -> book.getBookOnHands().equals(filterOnHands.getValue()));
        });
